...
function resolveKeyframe(kf: any, resolveFit: FitResolver): { center: Vec2; zoom: number } | null {
...
export function poseAt(track: CameraTrack, t: number, resolveFit: FitResolver, canvasW: number, canvasH: number): CameraPose {
  const kfs = track.keyframes as any[];
  if (kfs.length === 0) return { x: 0, y: 0, zoom: 0.5, azimuth: 0, polar: 0.06 };
  const resolved = kfs.map((kf) => resolveKeyframe(kf, resolveFit));
  let i = kfs.length - 1;
