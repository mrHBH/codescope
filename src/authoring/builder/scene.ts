...
export class SceneBuilder {
  /** @internal */ readonly doc: SceneDoc;
  ...
  /** @internal */ uid(base: string): string {
    const n = this.idCounter.get(base) ?? 0;
    this.idCounter.set(base, n + 1);
    return `${base}_${n}`;
  }

  /** @internal */ addChild(parentId: string, childId: string) {
    (this.doc.objects[parentId] as GroupSpec).children.push(childId);
  }
...
export class ChapterBuilder {
  ...
  text(id: string | null, content: string, opts: { at: Vec2; size: number; color: Color; weight?: number; align?: 'left' | 'center' | 'right'; opacity?: number }) {
    ...
    this.s.addChild(this.grpId, oid);
    return oid;
  }
  ...
  private pUid(base: string): string { return this.s.uid(base); }
  ...
  cam = {
    ...
    dive: (opts: { into: Vec2; zoom: number; hold?: number; d?: number; box?: { at: Vec2; size: Vec2 } }) => {
      ...
      this.s.doc.camera.keyframes.push({ time: this.start, fit: this.grpId, ease: 'easeInOutCubic' });
      this.s.doc.camera.keyframes.push({ time: this.start + h, fit: this.grpId, ease: 'smoothstep' });
    ...
    diveObj: (opts: { target: string; zoom: number; hold?: number; d?: number }) => {
      ...
      this.s.doc.camera.keyframes.push({ time: this.start, fit: this.grpId, ease: 'easeInOutCubic' });
      this.s.doc.camera.keyframes.push({ time: this.start + h, fit: this.grpId, ease: 'smoothstep' });
      this.s.doc.camera.keyframes.push({
        time: this.start + h + d,
        fitObj: opts.target,
        zoomMul: 1 + opts.zoom,
        ease: 'smoothstep',
...
export class PageBuilder {
  ...
  private uid(base: string): string { return this.s.uid(base); }
