...
- [x] `Mobject` gains `elevation`, `extrude`, `faceTilt` (animatable via
...
- [x] FX path unchanged and still wins when active (fireworks/physics keep working).
...
- [x] Polygon with `extrude>0` emits: top face (existing `polygonQuads`,
...
- [x] Shading reuses `LIGHT_DIR` Gouraud term from `space3d/project3d.ts`.
- [x] Static geometry uploads once (v1 perf lesson); elevation changes are the
      only per-frame cost.
...
- [x] Generalize the fireworks/logo per-instance 3D (the `fxXforms` write in
...
- [x] Generalize the 🗻 pattern (`ctx.tilt(polar)`, `surface3dDemo`) to every
...
- [x] Elevated objects cast soft shadows on the ground plane — resolve OQ-2
